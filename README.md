...
Simply download the program and run it. You may want to add the pseudo-it folder to your $PATH variable for ease of use.

## Dependencies

### Pseudo-it is implemented in Python 3.

Pseudo-it essentially runs a suite of common bioinformatics software to go from raw reads in FASTQ format with a reference assembly in FASTA format to a pseudo-assembly of the reads in FASTA format. These programs are:
...
All of these programs can easily be installed from [bioconda](https://anaconda.org/bioconda), and we will provide a pre-set environment shortly.
...
| -p [INT] | The MAX number of processes Pseudo-it can use. It is highly recommended to run this with multiple processes. See [Processes](#Processes) for more detail. |
| -bwa-t [INT] | The number of threads for BWA mem to use for each FASTQ library. See [Processes](#Processes) for more detail. Default: 1. |
| -gatk-t [INT] | The number of threads for GATK's Haplotype caller to use. See [Processes](#Processes) for more detail. Default: 4. |
...
In addition to our own efforts to speed up the programs that pseudo-it runs, those programs themselves may also have the option to specify multiple processes. This means that the allocation of processes across programs can be confusing, and this section will try to explain how this is done.
...
2. BWA mem, the algorithm used to map reads by default, also allows for multiple processes with its internal `-t` option. Based on the number of read sets provided and the number of processes specified with `-p`, pseudo-it will automaticall determine how many threads BWA mem should use.

    For example, if one set of paired end reads were supplied (`-pe1` and `-pe2`) and `-p` was set to 9, BWA mem would use 9 all 9 threads for that single read set.

    If all three sets of reads are supplied (`-se`, `-pe1` and `-pe2`, and/or `-pem`) and `-p` was set to 9, BWA mem run all three sets simultaneously, each with 3 threads.
...
3. GATK's HaplotypeCaller also has a multi-thread option: `--native-pair-hmm-threads`. It is not clear how much this option effects runtime, but the default value with GATK is 4 which leave as default for pseudo-it. 
